...
public class RgbTeleporter implements ITeleporter {
    public static BlockPos thisPos = BlockPos.ZERO;
    public static boolean insideDimension = true;

    public RgbTeleporter(BlockPos pos, boolean insideDim) {
        thisPos = pos;
    ...
    public Entity placeEntity(Entity entity, ServerWorld currentWorld, ServerWorld destinationWorld,
        ...
        if (insideDimension) {
            ...
            for (BlockPos checkPos : BlockPos.getAllInBoxMutable(destinationPos.down(10).west(10), destinationPos.up(10).east(10))) {
                if (destinationWorld.getBlockState(checkPos).getBlock() instanceof BrpTeleporter) {
                    doSetBlock = false;
                    break;
                }
            }
            if (doSetBlock) {
                destinationWorld.setBlockState(destinationPos, ModBlocks.BRP_TELEPORTER.get().getDefaultState());
            }
        }
